<template>
  <div class="flex items-center justify-between">
    <h1 class="text-3xl my-4">Ruta List</h1>

    <nuxt-link
      :to="{ name: 'rutas-create' }"
      ...
      <tbody>
        <tr v-for="item in items" :key="item['@id']" class="border-b">
          <td class="px-6 py-4 text-sm">
            <nuxt-link
              :to="{ name: 'rutas-id', params: { id: getIdFromIri(item['@id']) } }"
          ...
          <td class="px-6 py-4 text-sm">
            <nuxt-link
              v-if="router.hasRoute('estacions-id')"
              :to="{ name: 'estacions-id', params: { id: item.estacion } }"
              class="text-blue-600 hover:text-blue-800"
            >
              {{ item.estacion }}
            </nuxt-link>
  ...
  <div v-if="view" class="flex justify-center">
    <nav aria-label="Page navigation">
      <ul class="flex list-style-none">
        ...
        <li :class="{ disabled: !pagination.next }">
          <nuxt-link
            :to="{ name: 'rutas-page-page', params: { page: pagination.last } }"
            :class="
              !pagination.next
                ? 'text-gray-500 pointer-events-none'
                : 'text-gray-800 hover:bg-gray-200'
...
import { useRutaListStore } from "~~/stores/ruta/list";
...
const rutaListStore = useRutaListStore();
const { items, view, error, isLoading, hubUrl } = await useFetchList<Ruta>(
  "rutas"
);
rutaListStore.setData({ items, view, error, isLoading, hubUrl });
...
useMercureList({ store: rutaListStore, deleteStore: rutaDeleteStore });
